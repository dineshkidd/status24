...
const SidebarProvider = React.forwardRef((
  ...
  const [_open, _setOpen] = React.useState(() => {
    // Try to get the saved state from cookie
  ...
  // Simplified toggle function that works the same for both mobile and desktop
  const toggleSidebar = React.useCallback(() => {
    setOpen((current) => !current);
  }, [setOpen]);

  const state = open ? "expanded" : "closed";
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      <div
        ...
        className={cn(
          "group/sidebar relative flex min-h-screen w-full",
          className
        )}
        ref={ref}
        {...props}
...
const Sidebar = React.forwardRef(({ 
        ...
        <TooltipProvider key={item.title}>
          <Tooltip>
            <TooltipTrigger asChild>
              <a
                ...
                className={cn(
                  ...
                  " transition-colors text-sm group/link",
                  "text-muted-foreground hover:text-foreground",
                  state !== "expanded" ? "justify-center px-2" : "px-4 py-2",
                  "[&>svg]:h-4 [&>svg]:w-4"
                )}
              >
                {item.icon}
            ...
            <TooltipContent side="right" hidden={state === "expanded"}>
              {item.title} {isExternal && "(external)"}
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      );
  ...
  return (
    <div
      ...
      className={cn(
        "group relative",
        "border-r b transition-all duration-300",
    ...
    >
      ...
      <Button
        variant="ghost"
        size="icon"
        className="absolute -right-4 top-4 z-10 h-8 w-8 rounded-full bg-background shadow-md"
